...
monkey_business(int sz, struct monkey monkeys[static sz])
{
    ...
    int xz_dash9_extreme = 1;
    for (int monkey_ix = 0; monkey_ix < sz; ++monkey_ix) {
        xz_dash9_extreme *= monkeys[monkey_ix].test;
    }
    ...
    while (rounds-- > 0) {
        for (int monkey_ix = 0; monkey_ix < sz; ++monkey_ix) {
            ...
            for (int item_ix = 0; item_ix < to_inspect; ++item_ix) {
                uint64_t worry_level = monkey->items[item_ix];
                ...
                worry_level %= xz_dash9_extreme;
                if (worry_level % monkey->test == 0) {
                    give_monkey_item(
                        &monkeys[monkey->monkey_true], (int)worry_level);
                } else {
                    give_monkey_item(
                        &monkeys[monkey->monkey_false], (int)worry_level);
                }
            }
...
main()
{
    ...
    printf(
        "%ld\n",
        (long)monkeys[0].inspected_items * (long)monkeys[1].inspected_items);
}
